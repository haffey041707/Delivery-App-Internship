...
function renderDriverOrderInsights(bookings){
  const available=bookings.filter(item=>item.status==='searching');
  const active=bookings.find(item=>['accepted','pickup','in_transit'].includes(item.status));
  const availability=document.getElementById('driverAvailabilityText');
  const availableCount=document.getElementById('driverAvailableCount');
  ...
  if(activeAction){
    activeAction.dataset.activeBooking=active?.id||'';
...
document.addEventListener('click', async event => {
  ...
  const activeTrip=event.target.closest('[data-active-booking]');
  if(activeTrip){
    const id=Number(activeTrip.dataset.activeBooking);
    ...
    home.innerHTML = `
      <div class="driver-home-shell">
        ...
        <section class="driver-home-insights">
          ...
          <article class="driver-summary-card driver-days-card"><div class="driver-card-heading"><div><small>MEMBER ACTIVITY</small><h2>Days with Haulr</h2></div><b>Since joining</b></div><div class="driver-days-chart" aria-label="18 days active"><svg viewBox="0 0 120 120" aria-hidden="true"><circle class="driver-days-track" cx="60" cy="60" r="45"></circle><circle class="driver-days-progress" cx="60" cy="60" r="45"></circle></svg><div><strong>18</strong><small>days active</small></div></div></article>
        </section>
